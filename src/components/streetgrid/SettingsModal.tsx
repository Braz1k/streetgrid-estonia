...
export function SettingsModal({ open, onClose }: Props) {
  const { profile, updateProfile, settings, updateSettings, selectedCarId, getOwnedVehicle } = useStreetGrid();
  const equipped = getVehicleById(selectedCarId);
  const level = getOwnedVehicle(selectedCarId)?.level ?? 1;
    ...
    >
      ...
      >
        ...
        <div className="overflow-y-auto px-5 py-4 space-y-5">
          ...
          <Section icon={Car} title="АКТИВНЫЙ АВТОМОБИЛЬ" accent="primary">
            {equipped && (
              <div
                className="flex items-center gap-3 px-3 py-3 rounded-2xl border"
                style={{
                  background:  `${equipped.color}12`,
                  borderColor: `${equipped.color}44`,
                }}
              >
                <span className="text-3xl">{equipped.emoji}</span>
                <div className="flex-1">
                  <div className="text-sm font-black" style={{ color: equipped.color }}>{equipped.name}</div>
                  <div className="text-[10px] text-muted-foreground mt-0.5">LVL {level} · на карте</div>
                </div>
              </div>
            )}
            <p className="text-[11px] text-muted-foreground mt-2 leading-relaxed">
              Смена автомобиля и прогрессия — во вкладке <span className="text-accent font-bold">ГАРАЖ</span>.
            </p>
          </Section>
